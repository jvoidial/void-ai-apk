...
echo "[3/6] Write VOIDAI GPT build.gradle (with OkHttp + JSON)"
...
echo "[4/6] AndroidManifest with VOIDAI GPT MainActivity"
...
echo "[5/6] Persona-based MainActivity with GPT-style remote backend"
...
echo "[5a/6] RecyclerView adapter + tidy, code-aware styling"
...
echo "[5c/6] Rename workflow + artifact to VOIDAI APK"
cd "$REPO_DIR"
...
git commit -m "VOIDAI GPT: native chat client with remote LLM backend"
git push

echo "Done. GitHub Actions will now build the VOIDAI GPT APK."
